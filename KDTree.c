					...
					KDTreeSplitMethod split_method, int upper_level_dim)
{
	...
	if(size == 1)
		ROOT_SET(-1, -1, NULL, NULL, GetKDArrayCopied_Arr(arr)[0]);
	...
	med_val = COOR_VAL_BY_ORDER(arr, mat, split_dim, RND_HALF(size)); 
	
	split(arr,split_dim,kdleft, kdright);
 	KD_S(check, kdleft, root->left, split_method, uppder_level_dim + 1, root);
 	KD_S(check, kdright, root->right, split_method, uppder_level_dim + 1, root);

	ROOT_SET( split_dim, med_val, root->left, root->right, NULL);
}
